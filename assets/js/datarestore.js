...
var splitName = function(getName,addClass,addLabel) {
	...
	{   
	 ...
	 for(var i=0; i<dataRecd[curVal].checklister.length; i++){
		...
		if(dataRecd[curVal].checklister[i].length>1)
		{
		$('<div/>',{class:'checkboxDiv',id:'checkboxDiv_'+i}).appendTo('#rowDiv_'+i);
		$('<input>').attr({type:'checkbox',value:'checkbox_'+curVal+'_'+i,checked:'true'}).prop("checked",true).css('display','none').appendTo('#checkboxDiv_'+i);
		}
		else
		{
		$('<div/>',{class:'checkboxDiv',id:'checkboxDiv_'+i}).appendTo('#rowDiv_'+i);
